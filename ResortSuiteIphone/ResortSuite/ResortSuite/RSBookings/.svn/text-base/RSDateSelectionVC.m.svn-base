...
- (void)viewDidLoad {
	...
	else  if (pickerType == TimePicker )    
    {
        ...
        NSLog(@"time before: %@",datePicker.date);
    ...
    else if(pickerType == DatePicker)
    {
        ...
        //------------------------                
        NSDate *pickerDate = datePicker.date;
        NSLog(@"date before: %@",datePicker.date);
        NSDateFormatter *dateFormatter = [[NSDateFormatter alloc] init];
...
{	
	...
	if (pickerType == DatePicker) {     
        ...
        {			
            NSLog(@"self time string = %@",self.timeString);
...
{
    NSLog(@"Start date %@ and EndDate %@",startTimeOfDay, endTimeOfDay );
...
-(NSDate *)dateFromString:(NSString *)stringDate {
    ...
    NSLog(@"DAte Picker from string %@", Date);
	[dateFormatter release];
	return Date;
